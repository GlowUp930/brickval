...
export function issueBulkRecoveryToken(userId: string | null, now = Date.now()): string {
  const payload = encode({ userId, expiresAt: Math.floor(now / 1000) + TOKEN_TTL_SECONDS });
