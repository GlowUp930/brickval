...
export function computePricing(
  ...
  const blSoldNewDetails = sortByMostRecentDate(
    toBlDetails(brickLinkData?.sold_new?.price_detail)
  );
  const blSoldUsedDetails = sortByMostRecentDate(
    toBlDetails(brickLinkData?.sold_used?.price_detail)
  );
