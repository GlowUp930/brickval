...
- Native Sentry starts during app initialization when the local release configuration provides `SENTRY_DSN`. It uses the app version/build as the release identifier, `production` as the environment, and does not send default PII or scan images. The DSN is intentionally kept out of tracked source configuration.
