...
The full 70-test Swift suite passed on the small iPhone and iPhone 17 Pro simulators on 2026-08-12, including bulk recovery state/layout contracts, direct-tap crop planning, collection capacity, scan usage, access-cohort assignment, cached entitlement handling, stale-policy migration, and captured-image layout regressions. Free and Pro gating states were also checked on the small iPhone and iPhone 17 Pro simulators.
...
- Treat the native SwiftUI source/UI baseline and App Store upload number as separate: the current interface is based on build 46, while build 124 is the current release because Apple build numbers cannot return to 46 after builds 100 and 101.
...
- New users can be assigned once to the persisted access experiment. The experiment remains implemented, but the production kill switch is currently off after the Superwall paywall data issue was found on 2026-08-14. The treatment requires Pro immediately before the first real scan; existing users and onboarding replays remain soft-access users. RevenueCat entitlement resolution must finish before the treatment gate can appear, and the server policy remains the kill switch. See [`docs/decisions/0001-new-user-access-experiment.md`](docs/decisions/0001-new-user-access-experiment.md).
- App Store Connect pricing is AUD 79.99 annual with a seven-day trial and AUD 9.99 monthly without a trial. RevenueCat's live `default` offering includes both products. Superwall campaign `101303` maps `onboarding_hard_access` to paywall `254889`; on 2026-08-14, that paywall was observed serving template placeholder copy, a false monthly trial option, and stale hardcoded pricing in production. The production hard-access experiment is disabled server-side pending a Superwall editor repair. The existing `brickval_upgrade` campaign and paywall `223721` remain unchanged.
