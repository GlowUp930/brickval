...
Release complete: Apple processed **1.0.8 (171)** (`cc796f1e-f5ff-4ec3-8c47-b12b157dc738`); both existing internal groups have access and testing notes are saved. Sentry visibly lists the matching BrickVal debug UUID `c77f9720-642f-3f18-b34b-1ab037d34678`, plus framework symbols. Product source `bfabdf7`. GitHub's first run passes purchase/unit/backend checks but repeats an older graph UI test's offscreen-tap failure. The test now scrolls outside the chart gesture and asserts hit targets before tapping; the updated test passes on both local sizes, with CI confirmation pending. Shipped product code is unchanged by this test correction.
