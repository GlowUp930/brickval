...
- Current release build number: `151`. Build `100` was uploaded from the wrong Expo project and cannot be deleted through the available App Store Connect API, so later native builds must continue from that App Store sequence. Build `112` is the final pre-monetization UI pass. Build `113` adds shared Pro labeling, one introductory free bulk scan, a 10-unique-item free collection limit, contextual upgrade flows, and server-controlled future scan/history experiments. Build `114` adds the collection-item removal redesign and hidden onboarding replay gesture. Build `115` adds guided on-device bulk detection. Build `116` adds recall-first bulk recovery, ambiguous-match review, and tap-to-recover for missed figures. Build `118` isolates tap-to-recover uploads around the selected figure to prevent nearby figures from being identified instead. Build `119` makes missed-figure recovery direct and unobstructed by removing broad tap guides from the photo. Build `120` fixes recovery candidate layout collapse, centers recovery crops on direct taps, and replaces the recovery circle with animated corner focus brackets. Build `121` returns manual recovery matches to the bulk review with a visible confirmation and the recovered card in view. Build `122` was the last App Store Connect upload for the validated pricing and paywall configuration. Build `124` updates the Sentry integration to preserve framework dSYMs for crash symbolication. Build `125` moves hard-access enforcement to the app root and prepares the persistent 50/50 new-user paywall experiment while preserving the staged 10 percent production allocation until this build is live. Build `126` adds the choice-first product feedback surveys and notification-safe survey delivery. Build `128` is the verified TestFlight build uploaded after the production Pro-entitlement reconciliation fix. Build `133` adds dense photo-library bulk scanning with tiled local detection, up to 40 regions, paginated results, and compact glow-based recovery selection. Build `134` wires the existing Sentry DSN into the native release, tags production events with `com.brickval.app@1.0.5+134`, and preserves app and framework dSYMs in the archive. Build `135` adds native Sentry reporting for handled bulk-photo failures and preserves imported photos and regions for retry after recoverable errors. Build `136` adds the full-screen Lot Reveal. Build `137` replaces the slow spotlight with a rapid whole-lot valuation sweep: compact total and progress pills, spatial price cascades, a compact result carousel, persistent detected figures, and the existing review/recovery controls. Build `139` was the native TestFlight archive for the previous bulk-scan implementation. Build `140` adds the YOLO v7 stock-photo Core ML detector to bulk camera and bulk photo-library detection only; the upload succeeded on 2026-08-21 and is processing in App Store Connect. Build `141` adds the immersive bulk reveal review flow; the archive uploaded successfully on 2026-08-21 and is processing in App Store Connect. Build `142` routes upgrades directly to the Superwall paywall and adds the remote minimum-build update gate; it was archived, signed, and uploaded on 2026-08-22 and is processing in App Store Connect. Build `143` keeps photo-library bulk imports visibly active while the picker data loads, preserves local and server limit failures on the scanner with retry context, and prevents late live-frame callbacks from overwriting an active import; it was archived, signed, and uploaded on 2026-08-22 and is processing in App Store Connect. Build `144` surfaces upgrade failures instead of silently ignoring them and removes the duplicate bulk status container; it was archived, signed, and uploaded on 2026-08-22 and is processing in App Store Connect. Build `145` is the TestFlight archive containing the published promo-code handler; it was archived, signed, and uploaded on 2026-08-24 and is processing in App Store Connect. Build `146` contains the simplified bulk price reveal and was archived, signed, and uploaded on 2026-08-24; App Store Connect accepted the package and it is processing. Build `148` contains the Social-First Bulk Value Reveal; it was archived, signed, and uploaded on 2026-08-26 after the full native suite passed, and App Store Connect accepted the package for processing. Build `149` contains the contextual bulk match correction flow; it was archived, signed, and uploaded on 2026-08-26 after the native suite passed, and App Store Connect accepted the package for processing. Build `150` contains the seed-29 YOLOv8n bulk export that was later found to expose a raw Core ML tensor instead of Vision object observations. Build `151` re-exports the same seed-29 weights with Core ML NMS enabled, validates the Vision object-detection contract at runtime, and is the corrected TestFlight release. The centered-stage profile editor remains available from Manage Account rather than the main Profile tab. App Store Connect version `1.0.5` is the active TestFlight train.
