...
Release complete: Apple processed **1.0.8 (171)** (`cc796f1e-f5ff-4ec3-8c47-b12b157dc738`); both existing internal groups have access and testing notes are saved. Sentry visibly lists the matching BrickVal debug UUID `c77f9720-642f-3f18-b34b-1ab037d34678`, plus framework symbols. Product source `bfabdf7`. GitHub's purchase/unit/backend checks pass; graph UI checks exposed a fresh-install tips panel covering the controls. The tests now force first-launch tips and dismiss “Got it” before asserting graph interaction. A local forced-first-launch failure confirmed this cause; final CI confirmation is pending. Shipped product code is unchanged by this test correction.
