...
Signed archive `/tmp/BrickVal172.xcarchive` succeeded with the normal Release configuration (no DEBUG fixture override), version 1.0.8 (172). Xcode Organizer uploaded it successfully through the valid Apple account session after the command-line exporter hit its stale account cache. App Store Connect processing is still pending. The three archive debug files are already present in Sentry; the temporary upload token was revoked immediately after verification. No backend deployment, migration, subscription product change, or pricing change is part of this task.
