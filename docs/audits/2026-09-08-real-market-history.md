...
The regional filter is general-purpose: it offers All regions plus the seller countries actually present in BrickLink completed-sale rows. The earlier United States choice was an example, not a product limit. The backend now normalizes and returns only the optional two-letter seller country code and uses `collection-history:v3` so older history responses are refreshed. Native saved sales decode the optional field safely; a local metadata version causes existing rows to refresh once after upgrade. Missing-country rows remain available under All regions and are excluded from every selected country.
