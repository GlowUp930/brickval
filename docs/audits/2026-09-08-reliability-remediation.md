...
| F06 | Reconcile authoritative RevenueCat/Stripe state, order provider updates and aggregate active providers; preserve Pro referral credits. | Controlled lifecycle/outage tests and real database ordering/aggregation pass. **Live RevenueCat server key is missing; existing Pro ownership must be reconciled before rollout.** |
...
| F14 | Add pinned CI actions and backend, localization, database and small-simulator checks; preserve complete native source snapshot. | Local checks pass. GitHub rejected the workflow upload because the current OAuth credential lacks workflow scope. The prepared workflow remains local; hosted CI execution and reproducible signed archive remain unverified. |
...
1. Restore the existing RevenueCat V1 server key into Vercel production. The existing dashboard key was located, but the Vercel browser session requires sign-in; no key was saved or revealed in output. Verify authoritative ownership of all existing Pro rows (10 at the last read) without revoking unresolved users.
